...
def _evaluate(
    ...
    links, invalid_link_pairs, link_codes = _analyze_links(snapshot, issues)
    codes.extend(item_codes)
    codes.extend(pull_codes)
    codes.extend(link_codes)
    invalid_pull_numbers = {*invalid_pull_items, *invalid_pulls}
    for number in sorted(pull_items.keys() & pulls.keys()):
        if pull_items[number].milestone_number != pulls[number].milestone_number:
            codes.append(f"pull.milestone_mismatch:{number}")
...
def _validate_pull_item(snapshot: ReleaseSnapshot, item: GitHubItem) -> tuple[str, ...]:
    ...
    if not isinstance(item.url, str) or not _is_direct_url(
        item.url, snapshot.repository_full_name, "pull", number
    ):
...
def _validate_pull(
    ...
    if not isinstance(pull.url, str) or not _is_direct_url(
        pull.url, snapshot.repository_full_name, "pull", number
    ):
        codes.append(f"pull.invalid_url:{number}")
    if pull.state not in _STATES:
        codes.append(f"pull.invalid_state:{number}")
    if pull.milestone_number is not None and not _is_issue_number(
        pull.milestone_number
    ):
        codes.append(f"pull.invalid_milestone:{number}")
...
def _analyze_links(
...
) -> tuple[tuple[ReleaseLink, ...], frozenset[tuple[int, int]], tuple[str, ...]]:
    ...
    for relation in snapshot.links:
        ...
        pull_number = relation.pull_request_number
        if not _is_issue_number(issue_number) or not _is_issue_number(pull_number):
    ...
    return tuple(valid), frozenset(invalid), tuple(sorted(set(codes)))
...
def _validate_link(snapshot: ReleaseSnapshot, relation: ReleaseLink) -> list[str]:
    ...
    if not isinstance(relation.url, str) or not _is_direct_url(
